...
def _node_to_roi(node_name, roi_prefix_map=None):
...
def _run_rm_on_aggregated_keys(
    ...
    for stat_key, part in df.groupby(key_col):
        seed = _edge_seed(str(stat_key), seed_base)
    ...
    if not res.empty:
        bh = _bh_fdr if bh_func is None else bh_func
